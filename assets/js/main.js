...
const ClockIn = {
    ...
    bindEvents: function() {
        $(document).on('click', '#submitClockIn', () => this.submit());
        $(document).on('keypress', '#clockInPassword', (e) => {
            if (e.which === 13) this.submit();
        });
        $(document).on('click', '#togglePassword', function() {
            const input = $('#clockInPassword');
    ...
    checkStatus: function() {
        Stand120.ajax('get_clock_in_status').then(response => {
            if (response.success && response.data.clocked_in) {
                $('#clockInStatus').hide();
                $('#clockInForm').hide();
                $('#clockInDone').show();
                $('#clockInDoneMessage').text('You clocked in today at ' + response.data.clock_time);
            } else {
                $('#clockInStatus').hide();
                $('#clockInForm').show();
                $('#clockInPassword').focus();
            }
        }).catch(() => {
            $('#clockInStatus').html('<p style="color: var(--danger-color);"><i class="fas fa-exclamation-triangle"></i> Failed to check status</p>');
        });
    },
    
    submit: function() {
        const password = $('#clockInPassword').val().trim();
        
        if (!password) {
            Stand120.showAlert('danger', 'Please enter your password');
            $('#clockInPassword').focus();
            return;
        }
        
        $('#submitClockIn').prop('disabled', true).html('<span class="loading-spinner"></span> Clocking in...');
        
        Stand120.ajax('submit_clock_in', {
            password: password,
            device_build: this.getDeviceBuild()
        }).then(response => {
            $('#submitClockIn').prop('disabled', false).html('<i class="fas fa-check-circle"></i> Clock In Now');
            
            if (response.success) {
                const data = response.data.data;
                $('#clockInForm').hide();
                $('#clockInSuccess').show();
                $('#clockInSuccessName').text(data.staff_name);
                $('#clockInSuccessTime').text(data.date + ' at ' + data.time);
                Stand120.showAlert('success', response.data.message);
            } else {
                Stand120.showAlert('danger', response.data?.message || 'Clock-in failed');
            }
        }).catch(() => {
            $('#submitClockIn').prop('disabled', false).html('<i class="fas fa-check-circle"></i> Clock In Now');
            Stand120.showAlert('danger', 'Failed to submit clock-in');
        });
    },
    
    getDeviceBuild: function() {
        // Compose a browser-based build fingerprint
        return navigator.userAgent.substring(0, 100);
    }
};

// ========================================
// Clock-In History Module (Admin)
// ========================================
const ClockInHistory = {
    ...
    loadHistory: function() {
        ...
        $('#currentMonthLabel').text(monthNames[this.currentMonth - 1] + ' ' + this.currentYear);
        
        $('#clockInHistoryContent').html('<div class="loading-spinner" style="margin: 40px auto;"></div><p style="text-align: center; color: var(--text-muted);">Loading records...</p>');
        
        Stand120.ajax('get_clock_in_history', {
            ...
            if (response.success) {
                ...
                $('#clockInHistoryContent').html('<p style="text-align: center; color: var(--danger-color);">Failed to load records</p>');
            }
        }).catch(() => {
            $('#clockInHistoryContent').html('<p style="text-align: center; color: var(--danger-color);">Failed to load records</p>');
        });
    ...
    renderHistory: function(data) {
        ...
        const allStaff = data.all_staff || [];
        const lateThreshold = data.late_threshold || '08:00';
        
        const dates = Object.keys(records).sort().reverse();
        
        if (dates.length === 0) {
            $('#clockInHistoryContent').html('<p style="text-align: center; color: var(--text-muted); padding: 40px 0;"><i class="fas fa-calendar-times" style="font-size: 2rem; display: block; margin-bottom: 12px;"></i>No clock-in records for this month</p>');
            return;
        }
        
        let html = '<div class="table-responsive"><table class="table">';
        html += '<thead><tr><th>Date</th><th>Staff</th><th>Time</th><th>Status</th></tr></thead><tbody>';
        ...
        dates.forEach(date => {
            ...
            dayRecords.forEach((record, idx) => {
                html += '<tr>';
                if (idx === 0) {
                    html += '<td rowspan="' + dayRecords.length + '" style="font-weight: 600; vertical-align: top;">' + formattedDate + '</td>';
                }
                html += '<td>' + Stand120.escapeHtml(record.staff_name) + '</td>';
                html += '<td>' + record.formatted_time + '</td>';
                if (record.is_late) {
                    html += '<td><span style="color: var(--danger-color); font-weight: 600;"><i class="fas fa-exclamation-circle"></i> Late</span></td>';
                } else {
                    html += '<td><span style="color: var(--success-color);"><i class="fas fa-check-circle"></i> On Time</span></td>';
                }
                html += '</tr>';
            });
            
            // Show absent staff
            const presentIds = dayRecords.map(r => parseInt(r.staff_id));
            allStaff.forEach(staff => {
                if (!presentIds.includes(parseInt(staff.id))) {
                    html += '<tr style="opacity: 0.5;">';
                    html += '<td></td>';
                    html += '<td>' + Stand120.escapeHtml(staff.full_name) + '</td>';
                    html += '<td>—</td>';
                    html += '<td><span style="color: var(--text-muted);"><i class="fas fa-times-circle"></i> Absent</span></td>';
                    html += '</tr>';
                }
            });
        });
        ...
        let lateCount = 0;
        dates.forEach(date => {
            totalEntries += records[date].length;
            records[date].forEach(r => { if (r.is_late) lateCount++; });
        });
        
        let statsHtml = '<div style="display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 20px;">';
        statsHtml += '<div style="background: rgba(var(--primary-rgb, 110, 86, 207), 0.1); padding: 12px 20px; border-radius: 12px; flex: 1; min-width: 120px; text-align: center;">';
        statsHtml += '<div style="font-size: 1.5rem; font-weight: 700; color: var(--primary-color);">' + totalDays + '</div>';
        statsHtml += '<div style="font-size: 0.8rem; color: var(--text-muted);">Active Days</div></div>';
        statsHtml += '<div style="background: rgba(40, 167, 69, 0.1); padding: 12px 20px; border-radius: 12px; flex: 1; min-width: 120px; text-align: center;">';
        statsHtml += '<div style="font-size: 1.5rem; font-weight: 700; color: var(--success-color);">' + totalEntries + '</div>';
        statsHtml += '<div style="font-size: 0.8rem; color: var(--text-muted);">Total Clock-Ins</div></div>';
        statsHtml += '<div style="background: rgba(220, 53, 69, 0.1); padding: 12px 20px; border-radius: 12px; flex: 1; min-width: 120px; text-align: center;">';
        statsHtml += '<div style="font-size: 1.5rem; font-weight: 700; color: var(--danger-color);">' + lateCount + '</div>';
        statsHtml += '<div style="font-size: 0.8rem; color: var(--text-muted);">Late Arrivals</div></div>';
        statsHtml += '</div>';
        
        $('#clockInHistoryContent').html(statsHtml + html);
    },
    
    exportCsv: function() {
        $('#exportCsv').prop('disabled', true).html('<span class="loading-spinner"></span> Exporting...');
        
        Stand120.ajax('export_clock_in_csv', {
            month: this.currentMonth,
            year: this.currentYear
        }).then(response => {
            $('#exportCsv').prop('disabled', false).html('<i class="fas fa-download"></i> Export CSV');
            
            if (response.success && response.data.csv_data) {
                const rows = response.data.csv_data;
                ...
                a.href = url;
                a.download = 'clock-in-' + this.currentYear + '-' + String(this.currentMonth).padStart(2, '0') + '.csv';
                a.click();
                URL.revokeObjectURL(url);
                
                Stand120.showAlert('success', 'CSV exported successfully');
            }
        }).catch(() => {
            $('#exportCsv').prop('disabled', false).html('<i class="fas fa-download"></i> Export CSV');
            Stand120.showAlert('danger', 'Failed to export CSV');
        });
    },
    
    loadSettings: function() {
        Stand120.ajax('get_clock_in_settings').then(response => {
            if (response.success) {
    ...
    saveSettings: function() {
        $('#saveDeviceSettings').prop('disabled', true).html('<span class="loading-spinner"></span> Saving...');
        
        Stand120.ajax('save_clock_in_settings', {
            device_ip: $('#deviceIp').val().trim(),
            device_build: $('#deviceBuild').val().trim(),
            late_threshold: $('#lateThreshold').val().trim()
        }).then(response => {
            $('#saveDeviceSettings').prop('disabled', false).html('<i class="fas fa-save"></i> Save Settings');
            
            if (response.success) {
                Stand120.showAlert('success', response.data.message);
            } else {
                Stand120.showAlert('danger', response.data?.message || 'Failed to save settings');
            }
        }).catch(() => {
            $('#saveDeviceSettings').prop('disabled', false).html('<i class="fas fa-save"></i> Save Settings');
            Stand120.showAlert('danger', 'Failed to save settings');
        });
    }
};
