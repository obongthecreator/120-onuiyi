...
class Stand120_Ajax_Handler {
    ...
    public static function handle() {
        ...
        switch ($action) {
            ...
            // Clock-in actions
            case 'submit_clock_in':
                self::submit_clock_in();
                break;
            case 'get_clock_in_status':
                self::get_clock_in_status();
                break;
            case 'get_clock_in_history':
                self::get_clock_in_history();
                break;
            case 'export_clock_in_csv':
                self::export_clock_in_csv();
                break;
            case 'save_clock_in_settings':
                self::save_clock_in_settings();
                break;
            case 'get_clock_in_settings':
                self::get_clock_in_settings();
                break;
     ...
     * Submit clock-in
     */
    private static function submit_clock_in() {
        $data = array(
            'password' => $_POST['password'] ?? '',
            'device_build' => sanitize_text_field($_POST['device_build'] ?? '')
        );
        
        $result = Stand120_Clock_In::submit($data);
     ...
     */
    private static function get_clock_in_status() {
        $status = Stand120_Clock_In::get_today_status();
     ...
     */
    private static function get_clock_in_history() {
        if (!Stand120_Auth::is_admin()) {
            wp_send_json_error(array('message' => 'Unauthorized'));
            return;
        }
        ...
        $result = Stand120_Clock_In::get_history($filters);
        wp_send_json_success($result);
    }
     ...
     */
    private static function export_clock_in_csv() {
        if (!Stand120_Auth::is_admin()) {
            wp_send_json_error(array('message' => 'Unauthorized'));
            return;
        }
        ...
        $csv_data = Stand120_Clock_In::get_export_data($filters);
        wp_send_json_success(array('csv_data' => $csv_data));
    }
    
    /**
     * Save clock-in device settings (admin only)
     */
    private static function save_clock_in_settings() {
        if (!Stand120_Auth::is_admin()) {
            wp_send_json_error(array('message' => 'Only admins can change clock-in settings'));
            return;
        }
        
        $data = array(
            'device_ip' => sanitize_text_field($_POST['device_ip'] ?? ''),
            'device_build' => sanitize_text_field($_POST['device_build'] ?? ''),
            'late_threshold' => sanitize_text_field($_POST['late_threshold'] ?? '08:00')
        );
        
        $result = Stand120_Clock_In::save_device_settings($data);
    ...
    /**
     * Get clock-in device settings (admin only)
     */
    private static function get_clock_in_settings() {
        if (!Stand120_Auth::is_admin()) {
            wp_send_json_error(array('message' => 'Unauthorized'));
            return;
        }
        
        $settings = Stand120_Clock_In::get_device_settings();
